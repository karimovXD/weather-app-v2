    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const temperatureData = data.hourly.temperature2m.slice(0, next24Hours.length);
                const apparentTemperatureData = data.hourly.apparentTemperature.slice(0, next24Hours.length);
